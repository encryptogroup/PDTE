...
uint32_t estimateGates(uint32_t u, uint32_t v) {
	if (u == 1)
		return 0;
	else {
		uint32_t s1count = u/2;
		uint32_t s2count = (u % 2 == 0 && v % 2 == 0) ? v/2-1 : v/2;
		return (s1count + s2count) + estimateGates(u/2, v/2) + estimateGates(u-(u/2), v-(v/2));
	}
}
...
void PermutationNetwork::WaksmanPermutation::program_rec(uint32_t in, uint32_t block, uint32_t* p1, uint32_t* p2, uint32_t* rows, uint32_t* cols) {
	...
	if (newout < m_nNumOutputs && cols[newout] != -1) {
...
void PermutationNetwork::WaksmanPermutation::program(uint32_t* perm) {
	...
	uint32_t* cols = (uint32_t*) malloc(sizeof(uint32_t) * m_nNumOutputs); //new uint32_t[v];
	for (uint32_t i = 0; i < m_nNumOutputs; i++) cols[i] = -1;
	for (uint32_t i = 0; i < m_nNumInputs; i++) {
		uint32_t x = perm[i];
		assert(x>=0 && x<m_nNumOutputs && cols[x]==-1);
		cols[x] = i;
	}
	...
	if (m_nNumOutputs % 2 == 1) { // case b+d
			if(cols[m_nNumOutputs-1]!=-1) program_rec(cols[m_nNumOutputs-1], 1, p1, p2, rows, cols);
	}
	if (m_nNumInputs % 2 ==0 && m_nNumOutputs % 2 == 0) { // case a
			if(cols[m_nNumOutputs-1]!=-1) program_rec(cols[m_nNumOutputs-1], 1, p1, p2, rows, cols);
			if(cols[m_nNumOutputs-2]!=-1) program_rec(cols[m_nNumOutputs-2], 0, p1, p2, rows, cols);
	}
...
PermutationNetwork::WaksmanPermutation::WaksmanPermutation(uint32_t numinputs, uint32_t numoutputs, PermutationNetwork* pm) {
...
std::vector<std::vector<uint32_t> > PermutationNetwork::WaksmanPermutation::generateCircuit(std::vector<std::vector<uint32_t> > inputs, std::vector<std::vector<uint32_t> > outputs) {
	...
	std::vector<std::vector<uint32_t> > in_p1(m_nNumInputs / 2);
	std::vector<std::vector<uint32_t> > in_p2(sizeB2);

	//uint64_t s = 1;
	//share *shr_s = m_PM->m_cBoolCirc->PutINGate(1, s, 1, SERVER);



	// first row X
	std::vector<std::vector<uint32_t> > outtmp(2);
	...
	std::vector<std::vector<uint32_t> > out_p1 = b1->generateCircuit(in_p1, NON_INIT_DEF_OUTPUT);
	std::vector<std::vector<uint32_t> > out_p2 = b2->generateCircuit(in_p2, NON_INIT_DEF_OUTPUT);
