	...
	*/
	...
	public:
		...
		*/
		...
		/*
			encode message offline part
		 */
		void enc_off(CipherText& c, fp::RandGen rg = fp::RandGen()) const
		{
			Zn u;
			u.setRand(rg);
			mulG(c.c1, u);
		...
		/*
			encode message online part
			input : m
			output : c = (c1, c2) = (g^u, h^u f^m)
		 */
		void enc_on(CipherText& c, const Zn& m) const
		{
			Ec t;
			mulF(t, m);
			Ec::add(c.c2, c.c2, t);
